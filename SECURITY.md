...
- Hub 的微信和个人 Relay 使用系统原生凭据库。飞书仅通过固定版本的官方 CLI 调用，使用隔离配置目录和固定 profile；凭据由 CLI 保存，macOS/Windows 使用原生存储，Linux 使用本地加密文件。
- 飞书首次连接先通过应用信息接口确认本应用创建者，仅接受该身份在就绪提示后发送的新消息来确定接收会话；群模式同时验证机器人提及。后续入站必须匹配已保存的 owner 和私聊/群会话。
...
Weixin QR/验证码/首条激活消息、飞书应用创建与绑定消息、系统安全存储解锁、多设备决定、更新授权和 reset 确认
